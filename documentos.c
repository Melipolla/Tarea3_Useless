...
void cargarDoc(HashMap * documentos, HashMap * palabras){
    char scan[200];
    int i = 0;
    printf(" Ingrese los nombres de los archivos a cargar (separados por espacio): \n");
    gets(scan);
    FILE * archivo;
        ...
        while(fgets(linea, 1023, archivo) != NULL){
            while(txt(linea, j) != NULL){
                ...
                if(doc == NULL){
                    ...
                    insertMap(new->map, guardar, nueva);
                    insertMap(documentos, nombreArchivo, new);
                    j++;
                }else{
